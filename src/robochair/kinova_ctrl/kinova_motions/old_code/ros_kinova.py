...
# Add kortex_driver to path
kortex_driver_path = os.path.join(os.path.expanduser('~'), 'catkin_ws', 'src', 'ros_kortex', 'kortex_driver')
if kortex_driver_path not in sys.path:
    sys.path.insert(0, kortex_driver_path)
...
class FullArmMovement:
    ...
    def example_send_cartesian_pose(self):
        ...
        trajectory.waypoints.append(
            self.FillCartesianWaypoint(
                feedback.base.commanded_tool_pose_x+0.1,
    ...
    def example_send_joint_angles(self):
        ...
        #     angularWaypoint.angles.append(0.1)

        position = [0, 120.22, 160, 292.38, 275.29, 107.01, 80.1]



        for i in range(self.degrees_of_freedom):
            angularWaypoint.angles.append(position[i])
        ...
        # waypoint.oneof_type_of_waypoint.angular_waypoint = [angularWaypoint]
        ...
        print(req)
        # Send the angles
        rospy.loginfo("Sending the robot waypoints...")
        try:
            self.execute_action(req)
        except rospy.ServiceException:
    ...
    def move_by_cartesian_pose(self,poses:list):
        """
        Get the actual cartesian pose to increment it.\n
        You can create a subscriber to listen to the base_feedback.\n
        Here we only need the latest message in the topic though.\n

        Input: [x,y,z,theta_x,theta_y,theta_z]
        """
        assert len(poses) == 6, "Input 'poses' must contain exactly 6 values."
        ...
        trajectory.waypoints.append(
            self.FillCartesianWaypoint(
                feedback.base.commanded_tool_pose_x + poses[0],
                feedback.base.commanded_tool_pose_y + poses[1],
                feedback.base.commanded_tool_pose_z + poses[2],
                feedback.base.commanded_tool_pose_theta_x + poses[3],
                feedback.base.commanded_tool_pose_theta_y + poses[4],
                feedback.base.commanded_tool_pose_theta_z + poses[5],
                0)
        )
    ...
    def main(self):
        ...
        if success:
            ...
            success &= self.move_by_cartesian_pose([0.156674595508768, -0.0, -0.0,0,0,0])
            success &= self.move_by_cartesian_pose([0.086674595508768, -0.0, -0.0,0,0,0])
...
# position: [0.03648971779464101, 2.07785913571073, 2.8787447792694794, -1.1357130363475614, -1.4387876926174128, 1.845633486026638, 1.4318494921874838, 0.39825330183287405, 0.39825330183287405, -0.39825330183287405, 0.39825330183287405, 0.39825330183287405, -0.39825330183287405]
