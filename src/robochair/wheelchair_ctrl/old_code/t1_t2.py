...
class camera_wheelchair_teleop():
    def __init__(self) -> None:
        ...
        # PID controllers
        self.linear_pid = PID(Kp=35, Ki=0, Kd=0.1, setpoint=0)
        self.angular_pid = PID(Kp=160.0, Ki=0, Kd=1, setpoint=0)
    ...
    def execute_trajectory(self):
        ...
        trajectory_steps = [
            # new trajectory
            {"type": "move_x", "distance": 1.2, "timeout": 15},  # Turn left 30 degrees, 3-second timeout
...
#            {"type": "rotate", "angle": math.radians(-82.5), "timeout": 50},  # Turn left 30 degrees, 3-second timeout
            ...
            if not self.movement_complete:
                if current_step["type"] == "rotate":
                    if self.stage > self.stage_last:
                        ...
                        self.rotate_to_angle(self.rotate_by)
    ...
    def rotate_to_angle(self, target_angle):
        ...
        if abs(angular_velocity) < yaw_cap_low and abs(angular_velocity) > yaw_cap_low - 20:
