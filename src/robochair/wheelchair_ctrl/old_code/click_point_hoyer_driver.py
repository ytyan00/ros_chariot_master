...
class camera_wheelchair_teleop():
    def __init__(self) -> None:
        ...
        rospy.Subscriber('/master_next_point', String, self.master_next_point)
        self.pos_idx = None
        # publisher to rpi
        self.pub = pc_wheelchair_control_pub.WheelchairControlPublisher()
    ...
    def master_next_point(self,msg:String):
        try:
            self.flag_new_point = True
            self.pos_idx = msg.data
        except Exception as e:
            rospy.logerr("Error in wheelchair_callback: {}".format(e))
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if init_two:
                self.target_yaw = self.yaw_current + 30
            ...
            if (self.flag_new_point) and (self.start_time is not None):
                if (time.time() - self.start_time) > self.time_out:
                    # self.start_time = None
                    self.pub.wheelchair_publish("0,0")
                    # self.flag_new_point = False
