...
if __name__ == "__main__":
    try:
        ...
        while time.time() - start < 6.5:
            pos = hoyer_frame.get_dynamixel_position()
            time.sleep(0.1)
            try: 
                if random.choice([True, False]):
                    rint = random.randint(-150,-50)
                else:
                    rint = random.randint(50,150)
                new_pos = pos + rint
                hoyer_frame.set_dynamixel_position(motor_id=1, position=new_pos)
                time.sleep(1)
            except:
                print("error")
